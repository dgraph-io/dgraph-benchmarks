...
var blockSize = int(unsafe.Sizeof(UidBlock{}))

func FreePack(pack *UidPack) {
	if pack == nil {
		return
	}
	if pack.AllocRef == 0 {
...
func (e *Encoder) packBlock() {
	...
	block := (*UidBlock)(unsafe.Pointer(&b[0]))

	block.Base = e.uids[0]
	block.NumUids = uint32(len(e.uids))

	// block := &UidBlock{Base: e.uids[0], NumUids: uint32(len(e.uids))}
	last := e.uids[0]
	e.uids = e.uids[1:]
	...
	AssertTrue(sz == copy(block.Deltas, e.buf.Bytes()))
...
func (e *Encoder) Add(uid uint64) {
	if e.pack == nil {
		e.pack = &UidPack{BlockSize: uint32(e.BlockSize)}
...
func NewDecoder(pack *UidPack) *Decoder {
	decoder := &Decoder{
		Pack: pack,
	}
...
// Uids returns all the uids in the UidPack object as an array of integers.
...
func Encode(uids []uint64, blockSize int) *UidPack {
...
func EncodeFromBuffer(buf []byte, blockSize int) *UidPack {
	enc := Encoder{BlockSize: blockSize}
	var prev uint64
	for len(buf) > 0 {
...
// allocations.
func ApproxLen(pack *UidPack) int {
	if pack == nil {
		return 0
	}
...
func ExactLen(pack *UidPack) int {
	if pack == nil {
		return 0
	}
	sz := len(pack.Blocks)
...
func Decode(pack *UidPack, seek uint64) []uint64 {
...
func DecodeToBuffer(pack *UidPack, seek uint64) *z.Buffer {
...
func CopyUidPack(pack *UidPack) *UidPack {
	if pack == nil {
		return nil
	}

	packCopy := new(UidPack)
	packCopy.BlockSize = pack.BlockSize
	packCopy.Blocks = make([]*UidBlock, len(pack.Blocks))

	for i, block := range pack.Blocks {
		packCopy.Blocks[i] = new(UidBlock)
